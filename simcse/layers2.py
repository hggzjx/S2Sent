...
class NestedSelectiveAttention(nn.Module):
    def __init__(self, num, dim1, dim2, dct_n=6, dct_l=6, freq_sel_method='low4'):
        ...
        mapper_x = [temp_x * (dct_n // 6) for temp_x in mapper_x]
        mapper_y = [temp_y * (dct_l // 6) for temp_y in mapper_y]

        self.squeeze = MultiSpectralDCTLayer(dct_n, dct_l, mapper_x, mapper_y, dim1)
    ...
    def forward(self,hidden_states):
        ...
        b, d, n, l = stacked_out.shape
        stacked_out_pooled = stacked_out
        if n != self.dct_n or l != self.dct_l:
            stacked_out_pooled = torch.nn.functional.adaptive_avg_pool2d(stacked_out, (self.dct_n, self.dct_l))
